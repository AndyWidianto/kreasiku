...
const presenter = new FollowerPresenter({
    model: new data(),
    view: {
        followers: followers,
        loading: loading,
    ...
    observer = new IntersectionObserver((enteries) => {
        enteries.forEach(entry => {
            if (entry.isIntersecting) {
                presenter.getFollowers(username, limit.value, page.value, hasMore.value, followers.value, search.value);
            }
        })
    })
    if (loadMore.value) {
...
onMounted(async () => {
    await presenter.getFollowers(username, limit.value, page.value, hasMore.value, followers.value, search.value);
    await nextTick();
    initObserver();
})
watch(search, async (newValue) => {
    ...
    hasMore.value = true;
    followers.value = [];
    await presenter.getFollowers(username, limit.value, page.value, hasMore.value, followers.value, newValue);
    initObserver();
});
</script>
<template>
