...
function handleClick(e) {
    if (profile.value && !profile.value.contains(e.target)) {
        state.refDropDownProfile = false;
    }
    if (notif.value && !notif.value.contains(e.target)) {
        state.DropDownNotifikasi = false;
    }
}
onMounted(() => {
...
<template>
    <ul
        class="flex items-center gap-4 p-2 px-6 color-app border-b-1 border-gray-400 fixed left-[320px] top-0 w-[calc(100%-320px)] z-10">
        <li>
        ...
        <li class="ml-auto relative">
            <button class="flex items-center realtive" @click="handleShowDropDown">
                ...
                <div class="flex items-center absolute left-3 justify-center text-xss"
                    :class="[state.NitificationsNotRead.length !== 0 ? 'rounded-full w-4 h-4 p-1 bg-red-500 text-gray-100' : '']">
                    {{ state.NitificationsNotRead.length !== 0 ? state.NitificationsNotRead.length : '' }}
                </div>
            </button>
            <ul class="flex-col gap-2 p-2 bg-gray-100 absolute right-0 w-100 max-h-80 overflow-y-scroll scroll-hidden" ref="notif"
                :class="[state.DropDownNotifikasi ? 'flex' : 'hidden']">
                <li v-if="state.notifications.length < 1" class="w-full text-center p-2">
        ...
        <li class="shrink-0 w-10 h-10 relative" ref="profile">
            ...
            <ul class="absolute right-0 w-30 max-h-100 bg-gray-100 text-center"
                :class="[state.refDropDownProfile ? 'block' : 'hidden']">
                <li class="w-full">
