...
<template>
    ...
    <div class="w-full lg:w-2xl p-2" v-else>
        ...
        <div v-for="(posting, index) in postings" :key="posting?.posting_id" class="px-2">
            <RouterLink :to="`/profile/${posting?.user.username}`" class="flex items-center gap-3 p-2" :class="[index === 0 ? 'border-t-1 border-gray-600' : '']">
            ...
            <div class="w-full">
                ...
                <div v-if="posting?.images.length == 1" class="flex flex-wrap justify-around items-center">
                    <div class="flex items-center w-full max-h-150 bg-black">
                        <img v-for="image in posting.images" :src="image.image" :key="image.image_id"
                            alt="image posting" class="w-full min-h-60 max-h-100 object-cover">
                    </div>
                </div>
                <!-- untuk gambar 2 -->
                <div v-if="posting?.images.length < 3 && posting?.images.length > 1" class="flex flex-wrap justify-center items-center">
                    <div class="grid grid-cols-2 gap-[1px] w-full max-h-150 bg-black">
                ...
                <div v-if="posting?.images.length > 2" class="flex items-center justify-center">
                    <div class="grid grid-cols-3 gap-[1px] grid-rows-2 w-full bg-black max-h-100">
                        <img v-for="(image, indexImage) in posting.images.slice(0, 3)" :key="image.image_id"  @click="handleShowImage(posting.images, image.image_id)"
                            :src="image.image" alt="image posting" class="max-h-100 w-full object-cover"
