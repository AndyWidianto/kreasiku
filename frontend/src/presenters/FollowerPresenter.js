export default class FollowerPresenter {
    #model;
    #view;
    ...
    async getFollowers(username, limit, page, hasMore, followings, search) {
        if (!hasMore) return;
        this.#view.loading.value = true;
        try {
            const offset = (page - 1) * limit;
            const res = await this.#model.getFollowers(username, limit, offset, search);
            console.log(res);
